...
- **Identity is published before the count.** `activeTab(of:)` reads one tab; `tabsInFrontWindow(of:)` enumerates every tab in the window and is the half that can take a second on a many-tab browser — yet it only feeds the header's "n tabs open" and the `tabsOpen` metadata. So the enrichment resolves in two steps: read the active tab, publish `resolved` and flip `enrichmentFinished`, *then* read the count and publish `resolved` again. Nothing the plan depends on waits on the slow call.
- New state: `@State private var plan: CapturePlan?`, `@State private var shownNote: String?` (what the field was pre-filled with, if anything), `@State private var enrichmentFinished = false`, `@State private var saving = false`. `recording` is `model.recorder.isRunning` — the fact about whether spans get closed.
- `load()`: after `current = openActivity()`, set `plan = CaptureTargeting.plan(open: current, site: resolved.site, recording:, now:)` and, if `prefill(...)` is non-nil, set both `note` and `shownNote` to it (for a browser this is nil until the tab resolves). In the enrichment continuation, after `resolved = enriched`: recompute `plan`; if `note.isEmpty` and `prefill(...)` is non-nil, set `note` and `shownNote`; then the existing `recomputeSuggestion`. Order matters: a pre-fill that arrives suppresses the suggestion row, which is correct — the user already wrote about this span. `enrichmentFinished = true` is the last statement of the identity step, **outside** the `if enriched != snapshot` guard, so a non-browser app — where `resolvingBrowserTab` returns `self` unchanged — flips it too and never waits.
...
- `save()` stays synchronous (`.onSubmit` and the Escape button both take `() -> Void`): it checks `saving`, sets `saving = true` **before** spawning `Task { @MainActor in … }` — a `Task` body runs on a later main-actor hop, so setting the flag inside would let a second Return through — and the Task clears it with `defer`. Escape is ignored while `saving`, so a dismissal cannot race a write that is already committed to happening.
- Inside the Task, first **wait for the identity step, bounded to 1.5 seconds**: `let deadline = ContinuousClock.now + .seconds(1.5); while !enrichmentFinished, ContinuousClock.now < deadline { try? await Task.sleep(for: .milliseconds(20)) }`. A polled `@MainActor` flag, not a task handle: `await task.value` is not cancellation-responsive, and the enrichment's body is a synchronous `NSAppleScript` call whose Apple Event timeout is two minutes, so racing a handle against `Task.sleep` in a task group would still block on the group's implicit await — the panel would freeze for exactly the case the bound exists for. The wait matters because a one-word note plus Return can beat the tab read, and saving against an unresolved site would land on the previous tab: failure (2). On timeout, proceed with the current `resolved`; a late resolution afterwards is harmless (the field is non-empty, so no pre-fill fires, and on the failure path it improves the retry). Then re-fetch `open = try model.store.openActivity()` (the load-time value can be seconds stale), recompute `plan` from `resolved.site`, and resolve the target:
